...
#define MAX_INCOMING_BUFFER_SIZE 20000
...
Tester::MockDeframer::DeframingStatus Tester::MockDeframer::deframe(Types::CircularBuffer& ring_buffer, U32& needed) {
    needed = m_parent.m_next_needed;
    if (m_parent.m_next_needed < ring_buffer.get_remaining_size()) {
        return DEFRAMING_MORE_NEEDED;
    } else if (m_parent.m_inject_error != DeframingStatus::DEFRAMING_MAX_STATUS) {
        m_parent.m_remaining_size -= 1;
        return m_parent.m_inject_error;
    }
    Fw::Buffer buf = m_interface->allocate(needed);
    ring_buffer.peek(buf.getData(), needed, 0);
    m_interface->route(buf);
    m_parent.m_remaining_size -= needed;
...
Tester ::Tester(void)
    : DeframerGTestBase("Tester", MAX_HISTORY_SIZE),
      component("Deframer"),
      m_mock(*this),
      m_inject_error(DeframingProtocol::DeframingStatus::DEFRAMING_MAX_STATUS),
      m_remaining_size(0) {
    this->initComponents();
    this->connectPorts();
...
void Tester ::send_randomized_data(bool bad_receive) {
    U8 data[MAX_INCOMING_BUFFER_SIZE];
    ::memset(data, 0, MAX_INCOMING_BUFFER_SIZE);
    U32 i = 0;
    for (i = 0; i < STest::Pick::lowerUpper(0, MAX_INCOMING_BUFFER_SIZE)) {
        data[i] = static_cast<U8>(STest::Pick::lowerUpper(0, 255));
    }
    Fw::Buffer buffer(data, i);
    if (bad_receive) {
        // No calls should have been made in the case of an error
        invoke_to_framedIn(0, buffer, Drv::RecvStatus::RECV_ERROR);
        ASSERT_EQ(m_remaining_size, component.m_in_ring.get_remaining_size());
        ASSERT_from_comOut_SIZE(0);
        ASSERT_from_bufferOut_SIZE(0);
        ASSERT_from_bufferDeallocate_SIZE(0);
        ASSERT_from_framedDeallocate(0, buffer); // Still should deallocate buffer
    }else {
        m_remaining_size += i;
        invoke_to_framedIn(0, buffer, Drv::RecvStatus::RECV_OK);
        ASSERT_from_framedDeallocate(0, buffer); // Still should deallocate buffer
    }
}
...
void Tester ::test_incoming_frame() {
    this->send_randomized_data();
