...
namespace Svc {
    ...
    void PolyDb::getValue_handler(NATIVE_INT_TYPE portNum, U32 entry, MeasurementStatus &status, Fw::Time &time, Fw::PolyType &val) {
    ...
    void PolyDb::setValue_handler(NATIVE_INT_TYPE portNum, U32 entry, MeasurementStatus &status, Fw::Time &time, Fw::PolyType &val) {
        FW_ASSERT(entry < POLYDB_NUM_DB_ENTRIES,entry);
