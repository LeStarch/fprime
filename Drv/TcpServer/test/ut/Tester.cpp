...
void Tester ::transmit(Drv::TcpClientSocket& client) {
    ...
    for (U32 i = 0; i < STest::Pick::lowerUpper(1, 200); i++) {
        ...
        ASSERT_EQ(client.recv(buffer, size), SOCK_SUCCESS);
        validate_random_buffer(buffer);
        fill_random_buffer();
        ASSERT_EQ(client.send(m_data_buffer.getData(), m_data_buffer.getSize()), SOCK_SUCCESS);
...
bool Tester ::reconnect_or_fail(TcpClientSocket& client) {
    const U32 max_iteration = SOCKET_RETRY_INTERVAL_MS/10 + 2;
    for (U32 i = 0; i < max_iteration; i++) {
        if( client.open() == SOCK_SUCCESS) {
            return true;
        }
        Os::Task::delay(10);
    }
    EXPECT_TRUE(false) << "Failed to reconnect in reasonable time";
    return false;
}
...
void Tester ::test_basic_messaging(void) {
    ...
    this->component.configure("127.0.0.1", m_port);
    client.configure("127.0.0.1", m_port, 0, 100);
    status = this->component.startup();
    EXPECT_EQ(status, SOCK_SUCCESS);
    if (status == SOCK_SUCCESS) {
        this->component.startSocketTask(name, 99, 1024);
        // Faked retry
        if (not reconnect_or_fail(client)) {
...
void Tester ::test_basic_reconnect(void) {
    ASSERT_NE(m_port, 0);
    SocketIpStatus status = SOCK_SUCCESS;
    Drv::TcpClientSocket client;
...
void Tester ::test_advanced_reconnect(void) {
    ...
    if (status == SOCK_SUCCESS) {
        ...
        this->component.m_socket.close();
        // Enlongated (waited) retry
        Os::Task::delay((SOCKET_RETRY_INTERVAL_MS * 2) + 10);
        while (client.open() != SOCK_SUCCESS) {
        }
        transmit(client);
        client.close();
        this->component.stopSocketTask();
