...
void Tester ::transmit(Drv::TcpServerSocket& server) {
    ...
    for (U32 i = 0; i < STest::Pick::lowerUpper(1, 200); i++) {
        ...
        ASSERT_EQ(server.recv(buffer, size), SOCK_SUCCESS);
        validate_random_buffer(buffer);
        fill_random_buffer();
        ASSERT_EQ(server.send(m_data_buffer.getData(), m_data_buffer.getSize()), SOCK_SUCCESS);
...
void Tester ::test_basic_messaging(void) {
    ...
    server.configure("127.0.0.1", m_port, 0, 100);
    ASSERT_EQ(server.startup(), SOCK_SUCCESS);
...
void Tester ::test_basic_reconnect(void) {
    ...
    this->component.configure("127.0.0.1", m_port);
    this->component.startSocketTask(name, 99, 1024);
    // Random amount of retry failures
    for (U32 i = 0; i < STest::Pick::lowerUpper(1, 200); i++) {
        EXPECT_EQ(server.open(), SOCK_SUCCESS);
